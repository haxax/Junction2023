...
public class MapManager : Singleton<MapManager>
{
	...
	public Vector2 GetLocationAtWorldBounds()
	{
		Vector2 result = Vector2.zero;
		result.x = MapManager.Instance.CornerNodeCoordinate.Item1;
		result.y = MapManager.Instance.CornerNodeCoordinate.Item2;

		if (UnityEngine.Random.Range(0, 2) > 0)
		{
			result.x += UnityEngine.Random.Range(1, MapManager.Instance.FreeMapNodes.Count - 1);
			if (UnityEngine.Random.Range(100, 102) > 100) { result.y += MapManager.Instance.FreeMapNodes[0].Count - 2; }
			else { result.y += 1; }
		}
		else
		{
			result.y += UnityEngine.Random.Range(1, MapManager.Instance.FreeMapNodes[0].Count - 1);
			if (UnityEngine.Random.Range(100, 102) > 100) { result.x += MapManager.Instance.FreeMapNodes.Count - 2; }
			else { result.x += 1; }
		}
		return result;
	}
