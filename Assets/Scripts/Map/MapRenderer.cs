...
public class MapRenderer : MonoBehaviour
{
	...
	private void AddTileVertices(int x, int y, bool isFlipped)
	{
		...
		{
			// A
			_vertices.Add(new Vector3(x, y).ToPerlinHeight());
			_vertices.Add(new Vector3(x, y + 1).ToPerlinHeight());
			_vertices.Add(new Vector3(x + 1, y).ToPerlinHeight());

			// B
			_vertices.Add(new Vector3((x + 1), (y + 1)).ToPerlinHeight());
			_vertices.Add(new Vector3(x + 1, y).ToPerlinHeight());
			_vertices.Add(new Vector3(x, y + 1).ToPerlinHeight());
		}
		else
	...
	{
		if (!new Vector3(X + MapManager.Instance.CornerNodeCoordinate.Item1, Y + MapManager.Instance.CornerNodeCoordinate.Item2, 0).IsWithinBounds()) { return; }
		(int, int) xMinMax = (Mathf.Clamp(X - 1, 0, Nodes.Count), Mathf.Clamp(X + 1, 0, Nodes.Count - 1));
		(int, int) yMinMax = (Mathf.Clamp(Y - 1, 0, Nodes[0].Count), Mathf.Clamp(Y + 1, 0, Nodes[0].Count - 1));
